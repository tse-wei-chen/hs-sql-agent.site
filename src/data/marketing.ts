...
import {
  getMarketingPages as getBaseMarketingPages,
  getMarketingRoutes as getBaseMarketingRoutes,
  isMarketingSection as isBaseMarketingSection,
  normalizeMarketingLocale as normalizeBaseMarketingLocale,
} from "./marketing-base";
...
const aspNetCorePages: Record<MarketingLocale, MarketingPage> = {
  en: {
    locale: "en",
...
export function normalizeMarketingLocale(locale: string): MarketingLocale {
  return normalizeBaseMarketingLocale(locale);
}
...
export function getMarketingPages(
  ...
  return getBaseMarketingPages(locale, section).map(page =>
    page.section === "integrations" && page.slug === "aspnet-core"
      ? aspNetCorePages[normalizedLocale]
      : page
  );
}
...
export function isMarketingSection(value: string): value is MarketingSection {
  return isBaseMarketingSection(value);
}
