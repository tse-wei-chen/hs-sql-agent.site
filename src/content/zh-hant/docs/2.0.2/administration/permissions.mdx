---
title: 權限
description: 了解 hs-sql-agent 管理端採用的「權限路徑 + 動作」授權模型。
sidebar:
  group: 管理
  order: 79
---
...
不要從 `route.path` 推導授權，也不要假設 canonical permission path 可以直接透過 HTTP 呼叫。hs-sql-agent 的公開掛載是另一組固定契約：MCP 為 `/mcp`、管理 API 為 `/api`、管理 UI 為 `/`。
</Callout>
