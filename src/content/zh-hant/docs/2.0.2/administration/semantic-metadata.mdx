---
title: 語意中繼資料
description: 用資料表與欄位語意、關聯與指標中繼資料，強化 hs-sql-agent 的 schema discovery。
sidebar:
  group: 管理
  order: 76
---
...
hs-sql-agent 語意指標模型中的 `Executable` 固定為 `false`。指標說明可以協助 Agent 理解 schema 的業務含義，但不會變成可執行 SQL macro，也不會繞過 SQL 編譯器或授權流程。
</Callout>
...
hs-sql-agent 正式的 MCP 金鑰工具集合，是 MCP Tools Reference 列出的五個內建工具，再加上已發布的 Custom Tools。語意中繼資料由管理介面維護，再提供給 schema discovery 使用。
</Callout>
