---
title: 設定參考
description: hs-sql-agent 2.0.2 的 source-backed environment 與 runtime configuration reference。
sidebar:
  group: 維運
  order: 90
---
...
<CardGrid columns={3}>
  ...
  <DocCard title="Embedded .NET" icon=".NET" description="新整合使用 capability-specific options；HsSqlAgentServiceOptions 只保留給 legacy aggregate compatibility。" />
</CardGrid>
...
<Callout type="warning" title="v2.0.2 sample comment 提到 Delete，但 runtime code 使用 Purge">
`AuditRetentionService` 只接受 `Archive` 與 `Purge`。其他非 Archive 值（包含 `Delete`）會觸發 `AuditRetentionMode must be Purge or Archive.`。本 reference 以可執行的 v2.0.2 runtime code 為準，而不是 stale sample comment。
</Callout>
