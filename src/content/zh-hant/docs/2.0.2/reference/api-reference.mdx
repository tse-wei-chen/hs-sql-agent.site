...
description: 根據 hs-sql-agent 原始碼整理、供內建管理介面使用的 HTTP controller 路由參考。
sidebar:
  group: 參考
  order: 142
---
...
<Callout type="warning" title="這是 hs-sql-agent 的原始碼契約，不是獨立版本化的 REST 標準">
hs-sql-agent server 沒有把這些 controllers 放在 `/v1` 相容 namespace 下。外部整合應固定 hs-sql-agent 版本，升級時重新確認實際選用的 capability surface。
</Callout>
