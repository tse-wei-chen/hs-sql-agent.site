---
title: SQL 支援參考
description: hs-sql-agent 2.0.1 的 SQL 語法與六種資料庫 capability 支援摘要。
sidebar:
  group: SQL 編譯器
  order: 62
---

hs-sql-agent 2.0.1 不會把各資料庫完整的 SQL 語法直接視為可信任輸入。SQL 必須先能由 F# parser 表示、通過來源語意驗證、證明需要的 capability，並由對應 provider 執行階段編譯成不可變命令後才會執行。

本頁是 2.0.1 契約的易讀摘要。真正的權威仍是編譯器：不支援或需要特定執行階段 profile 的語法會以 fail-closed 方式拒絕，不會偷偷改寫成語意近似的另一段 SQL。
...
| Statement | 2.0.1 MCP DML 狀態 | 核准模型 |
| --- | --- | --- |
...
| `INSERT ... SELECT` | 拒絕 | 2.0.1 尚未定義來源資料列集合的核准語意，因此 fail closed |
...
下表整理 2.0.1 幾個重要差異；它不是 `CapabilityMatrix.fs` 全部 capability ID 的完整清單。
...
2.0.1 capability matrix 對 PostgreSQL、MySQL、SQLite 宣告可攜式 JSON extraction lowering。Canonical JSON-set mutation contract 則宣告於 PostgreSQL、MySQL、SQLite、SQL Server。
...
- MySQL 與 SQL Server 在 2.0.1 沒有宣告可攜式 target contract。

這正是「資料庫本身能執行」仍不足以代表 hs-sql-agent 應該接受該 SQL 的例子。

## JOIN 版本條件

SQLite `RIGHT JOIN` 與 `FULL JOIN` 需要 target capability profile 明確為 SQLite 3.39+。MySQL 的 `FULL JOIN` 在 2.0.1 capability matrix 中仍是 rejected。
...
其他 provider 目標對這些 canonical capability，在 2.0.1 matrix 中維持 rejected。
...
本頁不宣稱每個 function name、operator spelling、CAST target、JSON path、window frame 或 vendor extension 都已支援。2.0.1 capability matrix 比單一文件表格更細，而且包含執行階段版本條件。
