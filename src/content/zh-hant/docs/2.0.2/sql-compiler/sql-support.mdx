---
title: SQL 支援參考
description: hs-sql-agent 的 SQL 語法與六種資料庫 capability 支援摘要。
sidebar:
  group: SQL 編譯器
  order: 62
---
...
| Statement | hs-sql-agent MCP DML 狀態 | 核准模型 |
| --- | --- | --- |
...
| `INSERT ... SELECT` | 拒絕 | hs-sql-agent 尚未定義來源資料列集合的核准語意，因此 fail closed |
...
SQLite `RIGHT JOIN` 與 `FULL JOIN` 需要 target capability profile 明確為 SQLite 3.39+。MySQL 的 `FULL JOIN` 在 hs-sql-agent capability matrix 中仍是 rejected。
