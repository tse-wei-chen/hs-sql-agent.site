...
description: hs-sql-agent 2.0.2에서 테이블/컬럼 의미, 관계, 지표 메타데이터로 데이터베이스 탐색 정보를 보강합니다.
sidebar:
  group: 관리
  order: 76
---
...
2.0.2 semantic metric model의 `Executable`은 false입니다. Metric 설명은 에이전트가 schema 의미를 이해하도록 도울 수 있지만 실행 가능한 SQL macro가 되지 않으며 SQL compiler나 authorization pipeline을 우회하지 않습니다.
</Callout>
...
2.0.2의 공식 MCP key tool surface는 MCP Tools Reference에 나오는 다섯 개 내장 도구와 게시된 Custom Tool입니다. Semantic metadata는 Admin 제어 평면에서 관리하고 schema discovery가 소비합니다.
</Callout>
