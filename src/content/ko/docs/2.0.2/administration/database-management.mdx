---
title: 데이터베이스 관리
description: hs-sql-agent에서 외부에 노출할 데이터베이스 연결을 구성하고 운영합니다.
sidebar:
  group: 관리
  order: 72
---
...
hs-sql-agent는 공통 SQL provider runtime을 통해 다음 provider identifier를 지원합니다.
...
hs-sql-agent 데이터베이스 요청 모델에는 다음 필드가 있습니다.
...
hs-sql-agent Admin API는 Database Management 작업에 명시적인 권한을 적용합니다.
