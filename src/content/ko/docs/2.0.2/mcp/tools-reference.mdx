---
title: MCP 도구 레퍼런스
description: hs-sql-agent 2.0.2 내장 MCP 도구의 공식 계약, 파라미터, 응답, 권한, 위험 경계를 설명합니다.
sidebar:
  group: MCP
  order: 35
---
...
**MCP 키가 관리하는 공식 2.0.2 내장 도구 표면**에는 정확히 다섯 개 도구 이름이 있습니다.
...
**하나의 `SELECT` SQL statement**를 받습니다. 공개 2.0.2 도구 계약은 JOIN, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, CTE, subquery, UNION/INTERSECT/EXCEPT 같은 일반 Query 형태를 명시적으로 포함합니다.
...
2.0.2 저장소에는 semantic-management 구현이 있지만 공식 MCP key built-in registry와 Admin key-management surface는 이 페이지에 나열된 다섯 도구를 인식합니다. 따라서 공식 2.0.2 문서에서 semantic metadata는 **Admin/control-plane capability이며 schema discovery가 소비하는 정보**로 다룹니다.
