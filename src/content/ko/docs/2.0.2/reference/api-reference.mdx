...
description: 번들 Admin UI가 사용하는 hs-sql-agent source-backed HTTP controller surface를 정리합니다.
sidebar:
  group: 참조
  order: 142
---
...
이 페이지는 **hs-sql-agent 소스에 실제로 존재하는 controller route**를 나열하지만, controller 노출 여부는 선택한 capability에 따라 달라집니다. 번들 Admin UI는 일반적으로 내장 identity 구성을 사용하며, 기존 ASP.NET Core host에 임베드할 때는 다른 구성을 선택할 수 있습니다.

<Callout type="warning" title="이 문서는 hs-sql-agent 소스 계약이며 별도로 버전 관리되는 REST 표준이 아닙니다">
hs-sql-agent server는 이 controller들을 `/v1` compatibility namespace 아래에 노출하지 않습니다. 외부 연동은 hs-sql-agent version을 고정하고 업그레이드할 때 선택한 capability surface를 다시 확인해야 합니다.
</Callout>
...
hs-sql-agent에는 `GET /api/Credential/status`가 존재하며 간단한 Credential API running status를 반환합니다. 해당 controller 자체에는 `[Authorize]` attribute가 없습니다. 인증된 credential-management API가 아니라 범위가 매우 좁은 status endpoint로 취급하십시오.
...
외부 자동화에서 이 API를 사용한다면 hs-sql-agent를 고정하고 실제 소비하는 response model을 검증하십시오. 이 페이지는 route와 중요한 계약을 기록하지만 모든 Admin view model을 영구적인 external SDK 계약으로 간주하지 않습니다.
