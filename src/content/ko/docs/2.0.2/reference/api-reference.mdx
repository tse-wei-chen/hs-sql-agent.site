...
description: 번들 Admin UI가 사용하는 hs-sql-agent 2.0.1 source-backed HTTP controller surface를 정리합니다.
sidebar:
  group: 참조
  order: 142
---
...
이 페이지는 **hs-sql-agent 2.0.1 source에 실제로 존재하는 controller route**를 문서화합니다. 번들 Admin UI도 이 surface를 사용합니다.

<Callout type="warning" title="이것은 2.0.1 source contract이며 독립적으로 versioned된 REST 표준이 아닙니다">
2.0.1 server는 이 controller를 `/v1` compatibility namespace 아래에 노출하지 않습니다. 번들 Admin UI 외부의 integration은 hs-sql-agent version을 고정하고 upgrade 시 이 레퍼런스를 다시 확인해야 합니다.
</Callout>

## Authentication과 authorization

대부분의 Admin route는 인증된 Admin identity가 필요합니다. 보호된 runtime action에는 `/runtime/db-management` → `view`, `/auth/role` → `edit` 같은 path/action permission check가 추가로 적용됩니다.

Admin session이 생기기 전부터 시작되어야 하는 flow에는 익명 예외가 있습니다. First-run/sign-in/sign-up, 일부 OIDC endpoint, password recovery, MFA challenge completion 등이 이에 해당합니다.

자세한 내용은 [권한](/ko/docs/administration/permissions)을 참고하십시오.
...
2.0.1에는 `GET /api/Credential/status`가 존재하며 간단한 Credential API running status를 반환합니다. 해당 controller 자체에는 `[Authorize]` attribute가 없습니다. 인증된 credential-management API가 아니라 범위가 매우 좁은 status endpoint로 취급하십시오.
...
외부 자동화에서 이 API를 사용한다면 2.0.1을 고정하고 실제 소비하는 response model을 검증하십시오. 이 페이지는 route와 중요한 계약을 기록하지만 모든 Admin view model을 영구적인 external SDK 계약으로 간주하지 않습니다.
