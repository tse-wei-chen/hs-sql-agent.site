---
title: 설정 레퍼런스
description: hs-sql-agent의 source-backed 환경 변수 및 런타임 설정 레퍼런스입니다.
sidebar:
  group: 운영
  order: 90
---
...
저장소에 포함된 **hs-sql-agent `.env.example`**은 기본 Compose 경로에서 사용하는 배포용 환경 변수 목록입니다. 이 페이지는 환경 변수 이름은 해당 파일을 따르고, 주석과 validation이 충돌하면 hs-sql-agent 런타임 코드를 기준으로 합니다.
...
<Callout type="warning" title="hs-sql-agent 샘플 주석은 Delete라고 쓰지만 런타임 코드는 Purge를 요구합니다">
`AuditRetentionService`는 `Archive`와 `Purge`만 허용합니다. `Delete`를 포함한 다른 non-Archive 값은 `AuditRetentionMode must be Purge or Archive.` 오류를 발생시킵니다. 이 레퍼런스는 오래된 샘플 주석이 아니라 실행 가능한 hs-sql-agent 코드를 따릅니다.
</Callout>
...
Prometheus는 Admin/MCP application port와 **별도의 listener**에서 서비스됩니다. `OTLP_ENDPOINT`가 구성되면 hs-sql-agent는 SQL compile-evidence tracing/log integration을 포함한 telemetry를 export합니다.
