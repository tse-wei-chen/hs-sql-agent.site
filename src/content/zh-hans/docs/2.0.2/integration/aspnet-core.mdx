...
`HsSqlAgent.Server` 是可嵌入的 ASP.NET Core 类库包。hs-sql-agent 的新集成从无 options 的 core 开始，再明确选择宿主真正需要的 capability。
...
hs-sql-agent 中这些 mount 是刻意固定的。在 routing、frontend base path/assets、callback 和安全边界能够一起迁移之前，不宣传支持任意重定位。
