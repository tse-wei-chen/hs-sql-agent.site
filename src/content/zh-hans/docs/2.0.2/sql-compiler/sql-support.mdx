---
title: SQL 支持参考
description: hs-sql-agent 2.0.1 的 SQL 语法和 provider capability 人类可读摘要。
sidebar:
  group: SQL 编译器
  order: 62
---

hs-sql-agent 2.0.1 不会把数据库 provider 的完整 SQL 语法自动视为可信输入。只有当 F# parser 能表示该语句、source semantics 验证通过、所需 capability 可以被证明，并且 provider runtime 能把结果编译成 immutable command 时，SQL 才会被接受。

本页总结 2.0.1 契约。最终以编译器为准：不支持或受版本限制的结构会失败即拒绝，而不会被静默近似转换。
...
| `INSERT ... SELECT` | 拒绝 | 2.0.1 未定义 source row set 的审批语义，因此失败即拒绝 |
...
下面只列 2.0.1 的重要差异，不是 `CapabilityMatrix.fs` 所有 capability ID 的完整清单。
...
2.0.1 capability matrix 为 PostgreSQL、MySQL、SQLite 定义了可移植 JSON extraction lowering。canonical JSON-set contract 的 JSON mutation 则为 PostgreSQL、MySQL、SQLite、SQL Server 定义。
...
- MySQL 和 SQL Server 在 2.0.1 中没有声明该 capability 的可移植 target contract。

这就是为什么“数据库厂商自己能执行这条 SQL”并不足以让 hs-sql-agent 接受它。

## JOIN 版本门槛

SQLite 的 `RIGHT JOIN` 和 `FULL JOIN` 要求 target capability profile 为 SQLite 3.39 或更高。MySQL 的 `FULL JOIN` 在 2.0.1 capability matrix 中仍被拒绝。

无法证明所需 target contract 时，依赖这些 capability 的 Query 会被拒绝。

## DML returning 与 upsert

2.0.1 target contract 包括：
...
其他 provider target 在 2.0.1 matrix 中仍拒绝这些 canonical capability。
...
本页不会承诺每一个函数名、operator 写法、cast target、JSON path 形式、window frame 或 vendor extension。2.0.1 capability matrix 比单页表格详细得多，也包含运行时版本门槛。
