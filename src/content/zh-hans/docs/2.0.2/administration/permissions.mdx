---
title: 权限
description: 了解 hs-sql-agent 2.0.2 管理操作使用的 permission path + action 授权模型。
sidebar:
  group: 管理
  order: 79
---
...
不要从 `route.path` 推导授权，也不要假设 canonical permission path 可以直接通过 HTTP 调用。2.0.2 的公开挂载是另一组固定契约：MCP 为 `/mcp`、管理 API 为 `/api`、管理 UI 为 `/`。
</Callout>
