---
title: 配置参考
description: hs-sql-agent 基于源码的环境变量与运行时配置参考。
sidebar:
  group: 运维
  order: 90
---
...
仓库中的 **hs-sql-agent `.env.example`** 是默认 Compose 部署面向运维的环境变量清单。本页的环境变量名称以该文件为准；如果示例注释与运行时代码校验不一致，则以 hs-sql-agent 可执行代码为准。
...
<Callout type="warning" title="hs-sql-agent 示例注释写 Delete，但运行时代码要求 Purge">
`AuditRetentionService` 只接受 `Archive` 和 `Purge`；其他非 Archive 值（包括 `Delete`）都会抛出 `AuditRetentionMode must be Purge or Archive.`。本参考以可执行 hs-sql-agent 代码为准，而不是过时示例注释。
</Callout>
...
Prometheus 刻意运行在与 Admin/MCP 应用端口**独立的 listener**上。配置 `OTLP_ENDPOINT` 后，hs-sql-agent 会导出包括 SQL compile-evidence tracing/log integration 在内的遥测。
