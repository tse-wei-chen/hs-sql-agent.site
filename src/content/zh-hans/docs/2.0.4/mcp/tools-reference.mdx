---
title: MCP 工具参考
description: hs-sql-agent 2.0.4 的内置 MCP 工具契约；服务器会强制保持五项正式公开工具。
sidebar:
  group: MCP
  order: 35
---

MCP 内置工具刻意保持精简，从 2.0.4 起由服务器单一正式目录强制约束。公开的内置工具固定为五项：
...
`update_semantic_layer` 在 2.0.4 中不是内置 MCP 工具。Semantic Layer 属于控制平面配置，应通过管理界面或受权限保护的管理 API 编辑。
