...
description: Contrats des outils MCP intégrés de hs-sql-agent 2.0.4 ; la surface publique est imposée à cinq outils par le serveur.
sidebar:
  group: MCP
  order: 35
---

La surface MCP intégrée reste volontairement réduite. À partir de 2.0.4, un catalogue serveur unique impose exactement cinq noms d'outils publics.
...
`update_semantic_layer` n'est pas un outil MCP intégré en 2.0.4. La Semantic Layer appartient au plan de contrôle et se modifie via l'interface d'administration ou l'API d'administration protégée par permissions.
