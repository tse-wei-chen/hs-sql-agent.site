...
description: Surface réelle des contrôleurs HTTP de hs-sql-agent 2.0.2, issue du code source et utilisée par l’interface Admin intégrée.
sidebar:
  group: Référence
  order: 142
---
...
<Callout type="warning" title="Il s’agit d’un contrat source 2.0.2, pas d’un standard REST versionné séparément">
Le serveur 2.0.2 n’expose pas ces contrôleurs sous un namespace de compatibilité `/v1`. Les intégrations externes doivent fixer la version hs-sql-agent et revérifier la surface de capabilities sélectionnée lors d’une mise à niveau.
</Callout>
