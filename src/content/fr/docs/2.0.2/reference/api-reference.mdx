...
description: Surface réelle des contrôleurs HTTP de hs-sql-agent, issue du code source et utilisée par l’interface Admin intégrée.
sidebar:
  group: Référence
  order: 142
---
...
Cette page répertorie les routes de contrôleurs qui existent dans le **code source hs-sql-agent**, mais leur disponibilité dépend des capabilities sélectionnées. L’interface Admin intégrée utilise normalement la composition avec identité intégrée ; un hôte ASP.NET Core peut choisir une autre composition.

<Callout type="warning" title="Il s’agit d’un contrat source, pas d’un standard REST versionné séparément">
Le serveur n’expose pas ces contrôleurs sous un namespace de compatibilité `/v1`. Les intégrations externes doivent fixer la version hs-sql-agent et revérifier la surface de capabilities sélectionnée lors d’une mise à niveau.
</Callout>

<Callout type="note" title="L’identité intégrée et l’autorisation hôte n’exposent pas la même surface d’identité">
Avec `AddHsSqlAgentBuiltInAuth()`, les contrôleurs intégrés Auth, Member et Role font partie de la surface d’administration. Avec `AddHsSqlAgentHostAuthorization()`, HsSqlAgent n’expose intentionnellement **pas** `AuthController`, `MemberController` ni `RoleController`, n’installe pas son schéma d’identité et délègue les contrôles de permissions canoniques à l’application hôte. Les deux modes d’autorisation sont mutuellement exclusifs.
</Callout>
...
`GET /api/Credential/status` est disponible et retourne un simple état de fonctionnement de la Credential API. Le contrôleur lui-même ne possède pas d’attribut `[Authorize]`. Traitez-le comme un endpoint de statut étroit, pas comme une API authentifiée de gestion d’identifiants.
...
Pour l’automatisation cliente, fixez explicitement la version de hs-sql-agent et validez le modèle de réponse réellement consommé. Cette page documente les routes et contrats importants sans prétendre que chaque modèle de vue Admin constitue un SDK externe versionné de façon permanente.
