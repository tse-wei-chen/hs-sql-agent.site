...
description: Enrichir la découverte de base avec la sémantique des tables et colonnes, les relations et les métriques dans hs-sql-agent 2.0.2.
sidebar:
...
Le modèle sémantique 2.0.2 fixe `Executable` à false. Une description de métrique peut aider un agent à comprendre le schéma, mais ne devient pas une macro SQL exécutable et ne contourne ni le compilateur SQL ni le pipeline d’autorisation.
</Callout>
...
Pour la documentation 2.0.2, la surface formelle d’une clé MCP est constituée des cinq outils intégrés décrits dans la référence MCP, plus les Custom Tools publiés. Les métadonnées sémantiques sont administrées dans le plan de contrôle et consommées par la découverte de schéma.
</Callout>
