...
description: Enrichir la découverte de base avec la sémantique des tables et colonnes, les relations et les métriques dans hs-sql-agent.
sidebar:
...
| `Executable` | `false` dans le modèle |

<Callout type="warning" title="Les métriques sont des métadonnées, pas une échappatoire d’exécution">
Le modèle sémantique fixe `Executable` à false. Une description de métrique peut aider un agent à comprendre le schéma, mais ne devient pas une macro SQL exécutable et ne contourne ni le compilateur SQL ni le pipeline d’autorisation.
</Callout>
...
L’API expose la gestion sémantique sous `api/DbSemantic` :
...
Dans cette documentation, la surface formelle d’une clé MCP est constituée des cinq outils intégrés décrits dans la référence MCP, plus les Custom Tools publiés. Les métadonnées sémantiques sont administrées dans le plan de contrôle et consommées par la découverte de schéma.
</Callout>
