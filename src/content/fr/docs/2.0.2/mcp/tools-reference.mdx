...
description: Contrats formels des outils MCP intégrés de hs-sql-agent 2.0.1, paramètres, réponses, autorisation et frontières de risque.
sidebar:
  group: MCP
  order: 35
---
...
La **surface formelle des outils intégrés 2.0.1 gérée par les clés MCP** contient exactement cinq noms :
...
Chaque objet `ColumnInfo` 2.0.1 expose :
...
Accepte **une instruction SQL `SELECT`**. La description publique 2.0.1 mentionne explicitement JOIN, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, CTE, sous-requêtes et UNION/INTERSECT/EXCEPT.
...
Les classes d’instructions prises en charge dans le chemin MCP DML 2.0.1 sont :
...
| `INSERT ... SELECT` | refusée par défaut en 2.0.1 |
...
Le dépôt 2.0.1 contient une implémentation de gestion sémantique, mais le registre formel des outils intégrés et la gestion des clés Admin reconnaissent les cinq outils listés ici. La documentation traite donc les métadonnées sémantiques comme une **capacité du plan de contrôle consommée par la découverte de schéma**, et non comme un contrat MCP intégré supplémentaire.
