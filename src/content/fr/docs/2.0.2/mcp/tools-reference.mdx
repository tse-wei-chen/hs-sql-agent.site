...
description: Contrats formels des outils MCP intégrés de hs-sql-agent, paramètres, réponses, autorisation et frontières de risque.
sidebar:
  group: MCP
  order: 35
---
...
La **surface formelle des outils intégrés hs-sql-agent gérée par les clés MCP** contient exactement cinq noms :
...
Chaque objet `ColumnInfo` hs-sql-agent expose :
...
Accepte **une instruction SQL `SELECT`**. La description publique hs-sql-agent mentionne explicitement JOIN, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, CTE, sous-requêtes et UNION/INTERSECT/EXCEPT.
...
Les classes d’instructions prises en charge dans le chemin MCP DML hs-sql-agent sont :
...
| `INSERT ... SELECT` | refusée par défaut en hs-sql-agent |
...
Le dépôt hs-sql-agent contient une implémentation de gestion sémantique, mais le registre formel des outils intégrés et la gestion des clés Admin reconnaissent les cinq outils listés ici. La documentation traite donc les métadonnées sémantiques comme une **capacité du plan de contrôle consommée par la découverte de schéma**, et non comme un contrat MCP intégré supplémentaire.
