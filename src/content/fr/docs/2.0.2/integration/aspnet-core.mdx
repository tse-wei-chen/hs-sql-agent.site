...
`HsSqlAgent.Server` est une bibliothèque ASP.NET Core intégrable. Une nouvelle intégration part d’un core sans options puis sélectionne explicitement les capacités réellement nécessaires au host.
...
Ces montages sont volontairement fixes. Le déplacement arbitraire n’est pas annoncé comme pris en charge tant que routing, chemins de base/assets frontend, callbacks et frontières de sécurité ne peuvent pas être déplacés ensemble.
