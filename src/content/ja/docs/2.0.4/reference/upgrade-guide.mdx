...
Access Posture は発行／保存前の実効選択をそのまま可視化します。緑は read/query-only、黄は 1 つ以上の DML tool が有効、赤は tool scope が unrestricted であることを示します。さらに、全 table にアクセスできるか、現在の table whitelist 件数に制限されているかも表示します。これは説明用の UI であり、実際の authorization は既存の key／tool／table policy pipeline が引き続き強制します。
