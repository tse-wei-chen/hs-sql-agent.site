---
title: MCP ツールリファレンス
description: hs-sql-agent 2.0.4 の組み込み MCP ツール契約。公開面はサーバー側で 5 ツールに固定されます。
sidebar:
  group: MCP
  order: 35
---

組み込み MCP ツールは意図的に小さく保たれ、2.0.4 からはサーバー側の単一 catalog で強制されます。公開される組み込みツールは次の 5 つです。
...
`update_semantic_layer` は 2.0.4 の組み込み MCP ツールではありません。Semantic Layer は control plane の設定であり、Admin UI または権限保護された Admin API から編集します。
