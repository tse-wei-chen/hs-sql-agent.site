---
title: セマンティックメタデータ
description: hs-sql-agent 2.0.2 で、テーブル／カラムの意味、リレーション、メトリクスメタデータを使ってデータベース探索を補強します。
sidebar:
  group: 管理
  order: 76
---
...
2.0.2 のセマンティックメトリクスモデルでは `Executable` は false です。メトリクスの説明はエージェントがスキーマの意味を理解する助けになりますが、実行可能な SQL マクロになるわけではなく、SQL コンパイラや認可パイプラインも迂回しません。
</Callout>
...
2.0.2 の正式な MCP キーツールサーフェスは、MCP Tools Reference に記載された 5 つの組み込みツールと、公開済み Custom Tool です。セマンティックメタデータは管理コントロールプレーンから設定され、スキーマ探索で利用されます。
</Callout>
