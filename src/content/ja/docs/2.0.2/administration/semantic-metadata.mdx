---
title: セマンティックメタデータ
description: hs-sql-agent で、テーブル／カラムの意味、リレーション、メトリクスメタデータを使ってデータベース探索を補強します。
sidebar:
  group: 管理
  order: 76
---
...
| `Synonyms` | メトリクスの別名 |
| `Executable` | hs-sql-agent モデルでは `false` |

<Callout type="warning" title="メトリクスはメタデータであり、実行経路を迂回するものではありません">
hs-sql-agent のセマンティックメトリクスモデルでは `Executable` は false です。メトリクスの説明はエージェントがスキーマの意味を理解する助けになりますが、実行可能な SQL マクロになるわけではなく、SQL コンパイラや認可パイプラインも迂回しません。
</Callout>
...
hs-sql-agent の Admin API は `api/DbSemantic` 配下でセマンティック管理機能を公開します。
...
hs-sql-agent の正式な MCP キーツールサーフェスは、MCP Tools Reference に記載された 5 つの組み込みツールと、公開済み Custom Tool です。セマンティックメタデータは管理コントロールプレーンから設定され、スキーマ探索で利用されます。
</Callout>
