...
`HsSqlAgent.Server` は組み込み可能な ASP.NET Core クラスライブラリパッケージです。hs-sql-agent の新規統合では、オプションを持たない core から始め、ホストが必要とする capability を明示的に選択します。
...
これらの mount は hs-sql-agent では意図的に固定されています。routing、フロントエンドの base path／asset、callback、セキュリティ境界を一緒に移動できるようになるまでは、任意の場所への再配置をサポート対象として案内しません。
