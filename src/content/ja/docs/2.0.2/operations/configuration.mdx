---
title: 設定リファレンス
description: hs-sql-agent のソースに基づく環境変数とランタイム設定のリファレンスです。
sidebar:
  group: 運用
  order: 90
---
...
リポジトリに含まれる **hs-sql-agent `.env.example`** が、既定の Compose deployment で使用する環境変数一覧です。このページでは環境変数名についてそのファイルに従い、コメントと validation が食い違う場合は hs-sql-agent の実行コードを基準にします。
...
<Callout type="warning" title="hs-sql-agent の sample comment は Delete ですが、runtime code は Purge です">
`AuditRetentionService` が受け付けるのは `Archive` と `Purge` だけです。`Delete` を含む Archive 以外の無効値では `AuditRetentionMode must be Purge or Archive.` が発生します。このリファレンスでは古い sample comment ではなく、実行される hs-sql-agent code を基準にしています。
</Callout>
...
Prometheus は意図的に Admin/MCP application port とは**別の listener**で提供されます。`OTLP_ENDPOINT` を設定すると、hs-sql-agent は SQL compile-evidence の tracing / log integration を含む telemetry を export します。
