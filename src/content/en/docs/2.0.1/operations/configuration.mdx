...
description: Source-backed environment and runtime configuration reference for hs-sql-agent 2.0.1.
sidebar:
  group: Operations
  order: 90
---
...
<CardGrid columns={3}>
  ...
  <DocCard title="Embedded .NET" icon=".NET" description="HsSqlAgentServiceOptions is the code-level contract; its defaults are not always identical to Docker .env.example." />
</CardGrid>

The checked-in **v2.0.1 `.env.example`** is the deployment-facing inventory for the default Compose path. This page follows that file for environment names and uses the v2.0.1 runtime code when comments and validation disagree.
...
<Callout type="warning" title="The v2.0.1 sample comment says Delete, but runtime code says Purge">
`AuditRetentionService` accepts only `Archive` and `Purge`; any other non-Archive value, including `Delete`, raises `AuditRetentionMode must be Purge or Archive.` This reference follows executable v2.0.1 code rather than the stale sample comment.
</Callout>
...
Prometheus is intentionally served on a **separate listener**, not the Admin/MCP application port. When `OTLP_ENDPOINT` is configured, v2.0.1 exports telemetry including SQL compile-evidence tracing/log integration.
...
## Docker environment vs `HsSqlAgentServiceOptions`

The NuGet library exposes `HsSqlAgentServiceOptions` directly. **Do not assume every code default equals `.env.example`.** For example, the 2.0.1 service-options class has code defaults that differ from the Compose example for values such as bootstrap enablement, token lifetimes, health probes, and Prometheus.

When embedding the package:

1. treat `HsSqlAgentServiceOptions` as the code-level contract;
2. explicitly configure production-sensitive values rather than relying on defaults;
3. use `.env.example` as a deployment example, not as a declaration of every library default.
