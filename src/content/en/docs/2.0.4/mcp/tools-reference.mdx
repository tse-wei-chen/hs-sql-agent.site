...
description: Built-in MCP tool contracts for hs-sql-agent 2.0.4, with the five-tool public surface enforced by the server.
sidebar:
  group: MCP
  order: 35
---

The built-in MCP surface is intentionally small and is now enforced from one canonical server catalog. Version 2.0.4 exposes exactly five built-in tool names.
...
`update_semantic_layer` is not a built-in MCP tool in 2.0.4. Semantic metadata is a control-plane configuration surface and is edited through the Admin UI or permission-protected Admin API.
