...
If you run more than one application instance, process-local memory is no longer shared. Move the coordination-sensitive subsystems to their distributed providers and use a shared admin database. See [Distributed Deployment](/docs/operations/distributed-deployment).
