...
description: Source-backed environment and runtime configuration reference for hs-sql-agent.
sidebar:
  group: Operations
  order: 90
---
...
The checked-in **`.env.example`** is the deployment-facing inventory for the default Compose path. This page follows that file for environment names and uses the runtime code when comments and validation disagree.
...
<Callout type="warning" title="The sample comment says Delete, but runtime code says Purge">
`AuditRetentionService` accepts only `Archive` and `Purge`; any other non-Archive value, including `Delete`, raises `AuditRetentionMode must be Purge or Archive.` This reference follows executable runtime code rather than the stale sample comment.
</Callout>
...
Prometheus is intentionally served on a **separate listener**, not the Admin/MCP application port. When `OTLP_ENDPOINT` is configured, hs-sql-agent exports telemetry including SQL compile-evidence tracing/log integration.
