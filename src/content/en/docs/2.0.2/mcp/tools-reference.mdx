...
description: Formal hs-sql-agent built-in MCP tool contracts, parameters, responses, authorization, and risk boundaries.
sidebar:
  group: MCP
  order: 35
---
...
The **formal built-in tool surface managed by MCP keys** contains exactly five tool names:
...
Each `ColumnInfo` object exposes these properties:
...
Accepts **one `SELECT` SQL statement**. The public description explicitly calls out common query forms including JOIN, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, CTEs, subqueries, and UNION/INTERSECT/EXCEPT.
...
See [SQL Support Reference](/docs/sql-compiler/sql-support) for the human-readable capability summary.
...
Supported statement classes in the MCP DML path are:
...
| `INSERT ... SELECT` | rejected fail-closed |
...
Read [Safe DML](/docs/sql-compiler/safe-dml) for the full protocol.
