...
description: Enrich database discovery with table and column semantics, relationships, and metric metadata in hs-sql-agent 2.0.2.
sidebar:
...
| `Executable` | `false` in the 2.0.2 model |

<Callout type="warning" title="Metrics are metadata, not an execution bypass">
The 2.0.2 semantic metric model marks `Executable` false. A metric description can help an agent understand schema meaning, but it does not become an executable SQL macro and does not bypass the SQL compiler or authorization pipeline.
</Callout>
...
The 2.0.2 Admin API exposes semantic management under `api/DbSemantic`:
...
For 2.0.2 documentation, the formal MCP key tool surface is the five built-in tools listed in MCP Tools Reference plus published Custom Tools. Semantic metadata is managed through the Admin control plane and consumed by schema discovery.
</Callout>
