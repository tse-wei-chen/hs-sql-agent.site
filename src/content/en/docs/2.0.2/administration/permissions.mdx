---
title: Permissions
description: Understand the path-and-action authorization model used by hs-sql-agent Admin operations.
sidebar:
...
Do not derive authorization from `route.path` or assume a canonical permission path is callable over HTTP. In hs-sql-agent the public mounts are separate contracts: MCP is `/mcp`, the administration API is `/api`, and the administration UI is `/`.
</Callout>
