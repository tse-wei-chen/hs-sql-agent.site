...
description: Configure and operate database connections exposed through hs-sql-agent.
sidebar:
...
hs-sql-agent supports these provider identifiers through the shared SQL provider runtime:
...
The database request model contains these fields:
...
See [MCP Keys](/docs/administration/mcp-keys).
...
The Admin API applies explicit permissions to Database Management operations:
