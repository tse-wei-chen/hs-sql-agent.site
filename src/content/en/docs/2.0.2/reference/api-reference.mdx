...
description: Source-backed hs-sql-agent HTTP controller surface used by the bundled Admin UI.
sidebar:
  group: Reference
  order: 142
---
...
This page lists controller routes that exist in the **hs-sql-agent source**, but controller availability is capability-dependent. The bundled Admin UI normally uses the built-in identity composition; an embedding host can select a different composition.

<Callout type="warning" title="This is a source contract, not a separately versioned REST standard">
The server does not expose these controllers under a `/v1` compatibility namespace. External integrations should pin the hs-sql-agent version and re-check the selected capability surface when upgrading.
</Callout>
...
`GET /api/Credential/status` is available and returns a simple Credential API running status. The controller itself has no `[Authorize]` attribute. Treat it as a narrow status endpoint, not as an authenticated credential-management API.
...
For client automation, pin the release you deploy and validate the actual response model you consume. This page intentionally documents routes and important contracts without pretending that every Admin view model is a permanently versioned external SDK.
