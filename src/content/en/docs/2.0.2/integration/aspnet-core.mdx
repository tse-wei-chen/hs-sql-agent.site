...
`HsSqlAgent.Server` is an embeddable ASP.NET Core class-library package. New integrations start with an optionless core and explicitly select the capabilities the host needs.
...
These mounts are intentionally fixed. Arbitrary relocation is not advertised until routing, frontend base paths/assets, callbacks, and security boundaries can move together.
