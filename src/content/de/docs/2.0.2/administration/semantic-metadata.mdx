...
description: Datenbank-Ermittlung in hs-sql-agent um Bedeutungen von Tabellen und Spalten, Beziehungen und Metrik-Metadaten ergänzen.
sidebar:
...
| `Executable` | im Modell `false` |

<Callout type="warning" title="Metriken sind Metadaten, kein Ausführungs-Bypass">
Im semantischen Metrikmodell ist `Executable` auf `false` gesetzt. Eine Metrikbeschreibung kann einem Agent helfen, die Bedeutung des Schemas zu verstehen, wird aber weder zu einem ausführbaren SQL-Makro noch umgeht sie Compiler oder Autorisierung.
</Callout>
...
Die Admin API verwaltet Semantik unter `api/DbSemantic`:
...
In dieser Dokumentation besteht die formale Tool-Oberfläche aus den fünf integrierten Tools der MCP Tools Reference plus veröffentlichten Custom Tools. Semantische Metadaten werden über die Admin-Steuerung verwaltet und von der Schema-Ermittlung genutzt.
</Callout>
