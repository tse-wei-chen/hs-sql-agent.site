...
description: Formale Verträge der integrierten MCP-Tools in hs-sql-agent 2.0.1 mit Parametern, Antworten, Autorisierung und Risikogrenzen.
sidebar:
  group: MCP
  order: 35
---
...
Die **formale integrierte Tool-Oberfläche von 2.0.1, die durch MCP-Schlüssel verwaltet wird**, enthält genau fünf Tool-Namen:
...
Jedes `ColumnInfo`-Objekt in 2.0.1 enthält:
...
Akzeptiert **genau eine `SELECT`-SQL-Anweisung**. Die öffentliche Beschreibung in 2.0.1 nennt ausdrücklich häufige Formen wie JOIN, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, CTEs, Subqueries sowie UNION/INTERSECT/EXCEPT.
...
Unterstützte Statement-Klassen im MCP-DML-Pfad von 2.0.1:
...
| `INSERT ... SELECT` | in 2.0.1 fail-closed abgelehnt |
