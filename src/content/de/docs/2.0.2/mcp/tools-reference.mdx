...
description: Formale Verträge der integrierten MCP-Tools in hs-sql-agent mit Parametern, Antworten, Autorisierung und Risikogrenzen.
sidebar:
  group: MCP
  order: 35
---
...
Die **formale integrierte Tool-Oberfläche von hs-sql-agent, die durch MCP-Schlüssel verwaltet wird**, enthält genau fünf Tool-Namen:
...
Jedes `ColumnInfo`-Objekt in hs-sql-agent enthält:
...
Akzeptiert **genau eine `SELECT`-SQL-Anweisung**. Die öffentliche Beschreibung in hs-sql-agent nennt ausdrücklich häufige Formen wie JOIN, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, CTEs, Subqueries sowie UNION/INTERSECT/EXCEPT.
...
Unterstützte Statement-Klassen im MCP-DML-Pfad von hs-sql-agent:
...
| `INSERT ... SELECT` | in hs-sql-agent fail-closed abgelehnt |
